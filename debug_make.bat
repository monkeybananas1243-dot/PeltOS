...
%TOOLPATH%\i686-elf-ld.exe -T linker.ld kernel/kernel.o utils/get_char.o utils/io.o -o kernel/kernel.bin --oformat binary -Map kernel.map

copy /b boot\boot_sect.bin + kernel\kernel.bin boot\boot.bin

qemu-system-i386 -drive format=raw,file=boot.bin -d int,cpu_reset -D qemu.log
